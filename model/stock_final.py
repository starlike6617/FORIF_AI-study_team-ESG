...
class StockDataset(Dataset):
    def __init__(self, sentiment_path, ta_path, window_size=5):
        ...
        try:
            ...
            ta_cols = ["Close", "RSI", "SMA_5", "SMA_20", "EMA", "MACD", "Signal"]

            # 데이터 정규화 (각 컬럼별로 독립적으로)
            print("  - Normalizing data...", end="")
            self.scalers = {}
            normalized_data = []

            for col in sentiment_cols + ta_cols:
                scaler = MinMaxScaler()
    ...
    def __getitem__(self, idx):
        ...
        y = self.data[idx + self.window_size, 2]  # Close price
...
def main():
    ...
    for window_size in window_sizes:
        ...
        for company in companies:
            ...
            # 데이터 병합
            merged_df = pd.merge(sentiment_df, ta_df, on="Date", how="inner")
        ...
        # 전체 데이터에 대해 한 번에 정규화
        print("  - Normalizing combined data...", end="")
        scaler = MinMaxScaler()
        normalized_data = scaler.fit_transform(combined_df[sentiment_cols + ta_cols])
        print(" Done")

        # 데이터셋 생성
        dataset = torch.FloatTensor(normalized_data)

        # 시계열 데이터셋 생성 (window_size 적용)
        X, y = [], []
        for i in range(len(dataset) - window_size):
            X.append(dataset[i : i + window_size])
            y.append(dataset[i + window_size, 2])  # Close price
